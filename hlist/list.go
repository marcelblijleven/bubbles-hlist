...
// CursorLeft moves the cursor left. This can also move the state to the previous
// page.
...
// CursorRight moves the cursor right. This can also advance the state to the
// next page.
...
func (m *Model) updatePagination() {
	...
	// Restore index
	m.Paginator.Page = index / perPage
	m.cursor = index % perPage
...
func (m Model) View() string {
	...
	if m.showPagination {
		...
		pagination = lipgloss.Place(m.width, 1, lipgloss.Center, lipgloss.Center, pagination)
...
func (m Model) populatedView() string {
	...
	cells := make([]string, 0, len(pageItems))
	for i, item := range pageItems {
		...
		if i < len(pageItems)-1 && spacing > 0 {
			cell = lipgloss.NewStyle().MarginRight(spacing).Render(cell)
		}
